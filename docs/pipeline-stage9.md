...
- [x] **9.4.5 — RTK adapter** (~2.0 pw, HIGH WATCH — metering overlap with Pi-Mono)
  - [x] `ports/llm_proxy.py` Protocol (landed 2026-05-10 at commit `e195fcf`; 4 port-scoped DTOs PINNED — LLMRequest, LLMResponse, LLMStreamChunk, ProviderInfo — + Message at `ports/common.py` per A.1-WINSTON-1 + 2 error specializations CompressionContractViolation/CostFieldLeakage + `LLMProxyPort` Protocol with `API_VERSION="1.0.0"` + `@runtime_checkable`; per ADR-9.1.2-6 §3 + v0.2.3 §3.7 corrigendum closing F-9.4.5-LLM-DTO-01 at `de365ff`)
  - [x] Docker sidecar wiring — RETIRED at A.3 (`2043563`) per Q-9.4.5-21 H2-falsification (LiteLLM-vs-RTK ≥90% parity hypothesis retired); RTK falsified as LLMProxyPort substrate per B.1 preload empirical investigation (single-binary Rust CLI compression tool; no HTTP/gRPC; no chat-completion; no provider catalog); substrate compressed to LiteLLM-only PyPI library per ADR-9.2-V5 v0.6 stacked-corrigendum; RTK relocated out of 9.4.5 scope (A.3.α; revisit Stage 10+ as potential CLIToolingPort)
  - [x] Explicit cost-meter ownership ADR: RTK emits raw; Pi-Mono is the source of truth (substance INLINE per Q-9.4.5-3 — no separate ADR file — at port-contracts.md v0.2.3 ADR-9.1.2-6 §3.6; 3-layer defense-in-depth shipped at B.1 `9f6a010` — Layer 1 construction-time `VerdacaDTOMixin extra="forbid"` + Layer 2 port-boundary `CostFieldLeakage` guard + Layer 3 adapter-layer `_strip_cost_fields()` prefix-match — verified at B.2 `e9ea948` via M-T-LLM-COSTLEAK-01)
