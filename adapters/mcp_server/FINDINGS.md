...
## Verdaca-Specific Lessons (post-spike)

- Snapshot SHA at port time: `d93f71a`.
- Snapshot path of record: `tests/reference/spike-mcp-server-snapshot-d93f71a/`.
- The raw spike repository is an original source only; Stage 11 E2 reads and ports from the frozen snapshot.
- Verdaca maps the spike's tools/resources/prompts pattern onto five buyer-language tools, five resources, and one prompt.
- E2 transport work carries the caveat: A7 closed via team-lead override 2026-05-24; HYPOTHETICAL VOC substrate inherited.

- HARD-constraint grep at E2-H#3: `Select-String -Pattern '\bstrategic analysis\b'` returned zero hits in `tools.py`.
