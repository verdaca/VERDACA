...
# Stage 14 marker-bearing files carry no_waiver markers the Stage 13 collector's
# canonicalizer does not recognize (they are Stage 14 scope, mapped by the Stage 14
# collector instead). Exclude them so the Stage 13 inventory check does not
# false-positive on forward-stage markers.
STAGE_14_MARKER_PATHS: frozenset[Path] = frozenset(
    {
        CONTRACT_TESTS_ROOT / "ports" / "test_ast_gates_stage14.py",
        CONTRACT_TESTS_ROOT / "ports" / "test_jwks_rotation_invariant.py",
    }
)

EXCLUSIONS: frozenset[Path] = PRE_STAGE13_MARKER_PATHS | STAGE_14_MARKER_PATHS
